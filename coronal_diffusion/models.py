...
import config
from config import nmax, nlon, nlat
...
class DiffusionModel(nn.Module):
    ...
    def __init__(
        ...
        res_radius: float = config.res_radius,
    ):
        super().__init__()
        ...
        # Compute radii grid
        self.radii = torch.arange(min_radius, max_radius + res_radius, res_radius)
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, emb_dim, dropout=0.1):
        ...
        self.emb_mlp = nn.Sequential(
            nn.SiLU(),
            nn.Linear(emb_dim, out_channels * 2)  # *2 for scale and shift
        )
...
class DownBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, emb_dim, downsample=True, use_attention=False):
        super().__init__()
...
class UpBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, emb_dim, upsample=True, use_attention=False):
        super().__init__()
        self.resblock1 = ResidualBlock(in_channels, out_channels, emb_dim)
        self.resblock2 = ResidualBlock(out_channels, out_channels, emb_dim)
...
class MultispectralDiffusionUNet(nn.Module):
    ...
    def __init__(
        ...
        # Bottleneck
        self.bottleneck_res1 = ResidualBlock(channels[-1], channels[-1], combined_emb_dim)
        self.bottleneck_attn = AttentionBlock(channels[-1])
        self.bottleneck_res2 = ResidualBlock(channels[-1], channels[-1], combined_emb_dim)
